...
cat <<EOF
Additional fluidOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- fluidgerrit:   A Git wrapper that fetches/pushes patch from/to FluidOS Gerrit Review.
- fluidrebase:   Rebase a Gerrit change and push it again.
- fluidremote:   Add git remote for FluidOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for FluidOS Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/fluid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function fluidremote()
{
    ...
    local FLUID_USER=$(git config --get review.review.fluidos.org.username)
    if [ -z "$FLUID_USER" ]
    then
        git remote add fluid ssh://review.fluidos.org:29418/$PFX$PROJECT
    else
        git remote add fluid ssh://$FLUID_USER@review.fluidos.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/FluidOS/$PROJECT
...
function fluidgerrit() {
    ...
    local user=`git config --get review.review.fluidos.org.username`
...
function fluidrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "FluidOS Gerrit Rebase Usage: "
    ...
    git fetch "http://review.fluidos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
