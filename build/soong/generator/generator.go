...
func init() {
	android.RegisterModuleType("aosdp_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aosdpExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aosdpExpandVariables(ctx, dep_file)
	...
	cmd := aosdpExpandVariables(ctx, String(g.properties.Cmd))
