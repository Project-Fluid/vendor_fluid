...
func init() {
	android.RegisterModuleType("fluid_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = fluidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = fluidExpandVariables(ctx, dep_file)
	...
	cmd := fluidExpandVariables(ctx, String(g.properties.Cmd))
