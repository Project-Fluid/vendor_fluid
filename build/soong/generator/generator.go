...
func init() {
	android.RegisterModuleType("magma_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = magmaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = magmaExpandVariables(ctx, dep_file)
	...
	cmd := magmaExpandVariables(ctx, String(g.properties.Cmd))
