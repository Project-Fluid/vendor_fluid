...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:Project-Fluid-devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the fluid snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/fluid.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/fluid.dependencies'
    syncable_repos = []
    verify_repos = []
...
print("Repository for %s not found in the Fluid Devices Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
